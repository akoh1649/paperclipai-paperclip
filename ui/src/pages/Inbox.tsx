...
function runFailureMessage(run: HeartbeatRun): string {
  return firstNonEmptyLine(run.error) ?? firstNonEmptyLine(run.stderrExcerpt) ?? i18n.t("inbox:runExitedWithError", { defaultValue: "Run exited with an error." });
}
...
export function Inbox() {
  ...
  const retryRunMutation = useMutation({
    mutationFn: async (run: HeartbeatRun) => {
      ...
      if (!("id" in result)) {
        throw new Error(result.message ?? t("retryWasSkipped", { defaultValue: "Retry was skipped." }));
      }
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            title={nestingEnabled ? t("disableNesting", { defaultValue: "Disable parent-child nesting" }) : t("enableNesting", { defaultValue: "Enable parent-child nesting" })}
          >
            ...
            <PopoverContent align="end" className="w-40 p-2">
              <div className="space-y-0.5">
                {([
                  ...
                  ...(isolatedWorkspacesEnabled ? ([["workspace", t("groupWorkspace", { defaultValue: "Workspace" })] as const] as const) : []),
          ...
          <IssueColumnPicker
            ...
            title={t("chooseInboxColumns", { defaultValue: "Choose which inbox columns stay visible" })}
            iconOnly
          />
          {canMarkAllRead && (
            <>
